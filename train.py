...
from utils import get_model, cosine_sim, dot_sim,t_pro
...
def main():
    ...
    parser.add_argument('--val_step', default=25,type=int,
    ...
    parser.add_argument('--config', default='configs/hosa_f30k_MrSw.yaml',type=str, help="Which configuration to use. See into 'config' folder")
    ...
    # test_loader = data.get_test_loader(config, vocab=, workers=1, split_name='test')
    test_loader = data.get_test_loader(config, workers=1, split_name='test')
    ...
    for epoch in range(start_epoch, opt.num_epochs):
       ...
       # rsum,ndcg_sum = test(test_loader, model, tb_logger, measure=config['training']['measure'], log_step=opt.log_step,
 #                       ndcg_scorer=ndcg_val_scorer)

        # remember best R@ sum and save checkpoint
        is_best_rsum = rsum > best_rsum
        best_rsum = max(rsum, best_rsum)
        print("best R@1:",best_rsum) 
...
def validate(val_loader, model, tb_logger, measure='cosine', log_step=10, ndcg_scorer=None, alignment_mode=None):
   ...
   # print(x)
    ...
    # print("Average i2t Recall@k: %.1f" % mean_metrics[14])
...
def test(test_loader, model, tb_logger, measure='cosine', log_step=10, ndcg_scorer=None):
   ...
   # img_embs, cap_embs = encode_data(
    ...
    img_embs, cap_embs, img_lenghts, cap_lenghts = encode_data(model, test_loader, log_step, logging.info)
    ...
    tb_logger.add_scalar("Average i2t Recall@k: %.1f,",mean_metrics[15])
